    ...
    <div className="bg-white min-h-screen">
      <section className="bg-gradient-to-r from-nzx-green to-nzx-green-mid py-24 text-center px-4">
        <div className="max-w-4xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.05 }} className="font-body text-white/80 text-sm mb-6">
            <Link to="/products" className="hover:text-nzx-gold transition-colors">Products</Link>
            <span className="mx-2">/</span>
            <span className="text-white font-medium">{name}</span>
          </motion.div>
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="font-heading text-4xl md:text-6xl font-bold text-white mb-4">{name}</motion.h1>
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.15 }} className="font-body italic text-nzx-gold-light/90 text-xl font-medium">{tagline}</motion.p>
        </div>
      </section>
      ...
      <section className="py-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          ...
          <AnimatedSection delay={0.2} className="flex flex-col h-full justify-center">
            ...
            <p className="font-body text-nzx-mid text-lg leading-relaxed mb-10">{description}</p>
            <div className="mb-10">
              <h3 className="text-nzx-gold font-semibold tracking-widest text-xs uppercase mb-5">Key Benefits</h3>
          ...
          <AnimatedSection>
            ...
            <div className="mt-12 bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              <div className="grid grid-cols-3 bg-nzx-green text-white font-body font-semibold px-6 py-4">
                <div>Crop Type</div><div>Application Method</div><div>Recommended Dosage</div>
              </div>
            ...
            <p className="text-xs text-nzx-mid font-body text-center mt-4">* Note: The exact dosage should be tailored according to local soil tests and agronomist recommendations.</p>
          </AnimatedSection>
        </div>
      </section>

      <section className="py-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <AnimatedSection><SectionHeader label="Discover More" title="You May Also Like" /></AnimatedSection>
...
export const ProductPageTemplate = (props: ProductData) => (
  <PageTransition><OriginalTemplate {...props} /></PageTransition>
);
